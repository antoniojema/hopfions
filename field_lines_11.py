...
if ARG2 == 'sim':
	...
	if ARG1 == 'E':
		...
		if n == 60:
			...
			axe.seed.widget.position=[100.5,100.5,100.75]
			...
			axe = mlab.pipeline.streamline(field,seedtype='point',seed_visible=True,integration_direction='both',color=(0,0,139./255))
			axe.seed.widget.position=[100,100,70.25]
			axe.streamline_type='tube'
			axe.tube_filter.radius = 3
			axe.stream_tracer.maximum_propagation = 200
			axe.seed.widget.enabled=False
...
###################################################################### v TEORICO  v
	...
	if ARG1 == 'E':
		...
		if n == 60:
			axe = mlab.pipeline.streamline(field,seedtype='point',seed_visible=True,integration_direction='both',color=(0,0,139./255))
			axe.seed.widget.position=[100.5,100.5,100.75] #La diferencia de 0.25 aparece porque no estamos en t=0 exactamente
			axe.streamline_type='tube'
			axe.tube_filter.radius = 3
			axe.stream_tracer.maximum_propagation = 200
			axe.seed.widget.enabled=False
			...
			axe = mlab.pipeline.streamline(field,seedtype='point',seed_visible=True,integration_direction='both',color=(0,0,139./255))
			axe.seed.widget.position=[101,101,71.25]
			axe.streamline_type='tube'
			axe.tube_filter.radius = 3
			axe.stream_tracer.maximum_propagation = 200
			axe.seed.widget.enabled=False
